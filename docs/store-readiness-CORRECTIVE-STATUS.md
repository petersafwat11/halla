...
| EVT-02 | Atomic event grant/consume/refund | Backend | INTEGRATION_VERIFIED | `grantConsumable` race-routing + refund; `billing-fulfillment` (grant/guard/race/refund); **first-send `unused→consumed` trigger now covered** by `event-firstsend-consume.integration.test.js` (4: stamp+flip, no re-consume, isolation, already-consumed) — closes the Session-9 SBX-EVT-03 gap | `P0-03` dead-grant fixed | 2026-07-01 (trigger test 2026-07-02) |
...
| SEC-01 | Rotate/untrack/purge/secret scan | Owner/Ops | BLOCKED_NEEDS_OWNER | `evidence/store-readiness/SEC-01-OWNER-RUNBOOK.md`; `evidence/store-readiness/FINAL-GO-NO-GO.md` §2 | **THREE tracked secret-bearing files in history**: `config.env` (real secrets) + **`certs/mongodb-x509.pem` (MongoDB Atlas PRIVATE KEY — caught by the Session-10 audit; runbook now fixed to rotate+purge it)** + `halla-mobile/.env` (public WhatsApp number only, not a secret). Rotation + `git rm --cached` + history purge are OWNER-GATED (tracked `config.env` needed for boot). Evidence/metadata secret-free; no hardcoded reviewer password. | 2026-07-02 (cert 2026-07-02) |
