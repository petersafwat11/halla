...
- **DEVICE-ONLY first-verification — NO committed test locks the load-bearing claim: 2.** `SBX-OFR-01`
  (offers have no backend contract) and `FUNC-07` (push — no billing contract).
  - **`SBX-EVT-03` gap CLOSED (coordinator, 2026-07-02):** the `unused→consumed` **first-send trigger**
    (`messaging.send.service.js:293-314`) is now exercised by
    `test/event-firstsend-consume.integration.test.js` (4 cases: stamp+flip, no re-consume on a second
    send, subscription isolation, already-consumed untouched) — so on-device is now a CONFIRMATION of the
    real emission, not the first test of the flip. `FUNC-04` shares this now-covered trigger.

> Framing note (per §0.1): **no row is "fully proven by tests"** — every row's purchase/runtime act is
> DEVICE-ONLY. The 58 "confirms" rows mean *the backend interpretation is already code-verified, so
> on-device is a confirmation of the store's real emission, not the first test of the logic* (several
> carry an explicit per-row DEVICE-ONLY caveat for timing/enforcement/snapshot). The 2 DEVICE-ONLY rows
