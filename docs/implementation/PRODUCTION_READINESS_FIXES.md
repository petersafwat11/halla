...
| **B-2** Payment unit ambiguity | ✅ | `cc73d8d` + later SAR revert | Final contract: `pricing.oneTime` and `paymentProvider.charge`'s `amount` parameter are **SAR major units** (e.g. `29` = 29.00 SAR, `99.99` = 99.99 SAR). The provider converts SAR → halalas internally via `Math.round(amount * 100)` and validates that the input is finite, non-negative, and has at most 2 decimal places. PlanModel validator matches. (The original commit `cc73d8d` declared minor units; reversed in a follow-up commit because the entire codebase already stored SAR.) |
...
3. **Payment integration** — Plan / addon prices are stored in SAR (major units, e.g. `29` or `99.99`). The provider converts to halalas (Moyasar's required unit) internally before the API call. The validator rejects more than 2 decimal places. No operator action needed beyond verifying that real plan/addon prices in production look like SAR (not halalas).
