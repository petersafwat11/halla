...
### 12 September 2026 / Code review fixes and post-login UI redesign

- **Backend review fixes**: `imports.service.js` duplicate-key race now returns 409 `IDEMPOTENCY_CONFLICT` when the concurrent commit's CSV hash differs, instead of replaying another request's body. Gate resolve accepts a typed short code in `token` (normalized, event-scoped via `GuestsRepository.findByShortCode`); wire schema unchanged. New API test "gate: typed short code resolves within its event only". Contract table updated (`02-TECHNICAL-CONTRACT.md` §4).
- **UI redesign (all authenticated pages)**: semantic tokens in `web/styles/theme.css` (tokens.css untouched); rebuilt primitives (Button, Dialog, portal Menu, Pagination, Field/TextField, StatusBadge, Notice, EmptyState, Skeleton, IconButton); single-bar AppHeader with inline WorkspaceNav/EventSelector; guests page (EventHeaderBar with overflow menu, KPI StatsStrip, 5-column GuestTable with row action menus, mobile card rows); gate page (live counters, scan/lookup/camera card, sticky AdmissionCard states, recent list); all guest/event/import/export/QR/correction dialogs. New files: `GuestDeleteDialog.module.css`, `EventLifecycleDialog.module.css` (untracked — include when committing). All `data-testid`s preserved; row edit/correct/reset/delete and close/new-event moved behind menus, tests updated to open them first.
- **Accessibility**: `--ops-subtle` darkened to `#736b62` (≥4.5:1 on every surface token); Menu focuses its container when every item is disabled (closed event + admitted guest). Arabic glyph clipping in buttons fixed (label line-height).
- **Locales**: new keys added symmetrically to `en.json`/`ar.json`.
- **Tooling**: `scripts/demo-runner.mjs --dev` runs `next dev`; README documents `npm run demo` / `-- --dev`.
- **Verification (actual results)**: `npm --prefix api test` 70/70; contracts 41/41; `npm --prefix web test` 27/27; `npm --prefix web run test:browser` 12/12 (with `CHECKIN_EVIDENCE_DIR` pointed outside `docs/`); `npm run test:e2e` 2/2; `npm run lint` and `web lint` clean; `npm run design:check` passed; `next build` passed. Visual review via Playwright screenshots (AR/EN, 1440 px and 390 px) of guests, menus, draft/closed events, every dialog, gate idle/ready/success/already-admitted/invalid/search and reception role.
- **Not verified**: physical scanners, phones/cameras and printers; gate page now polls `/stats` every 5 s per device, which the 10 September capacity rehearsal did not include. Login page only lightly restyled. Nothing committed.
