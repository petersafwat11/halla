/**
 * Standalone create-event schema for the web app (labbe).
 *
 * Previously sourced from `@halla/shared-schemas`; now self-contained
 * for easier deployment. Web-specific bindings:
 *   - `buildDynamicTemplateSchema` injects live `FONT_IDS` from
 *     `@/config/fonts` (hydrated from `/api/v2/fonts`).
 *   - `timeAsDate` is false — web TimePicker emits `"HH:MM:AM"` strings.
 */
...
export const buildDynamicTemplateSchema = (fields, t) => {
  const fontIds = FONT_IDS && FONT_IDS.length > 0 ? FONT_IDS : DEFAULT_FONT_IDS;
  const timeAsDate = false;
