 ...
 * Guests service.
 *
 * Modeled on `ticketsService.js` — every call routes through `apiFetch`
 * so the in-memory access token is attached, 401 triggers a refresh +
 * replay, and the 30 s default timeout is inherited.
 *
 * Canonical mount is `/guests/...`; paths are sourced from
 * `ENDPOINTS.GUESTS.*` so this file owns no hardcoded URL strings.
 *
 * `guestFetch` prepends the `/guests` base path automatically (analogous
 * to the `authenticatedFetch` helper in `eventsService.http.js`) so call
 * sites read the route exactly as the backend declares it.
 */
