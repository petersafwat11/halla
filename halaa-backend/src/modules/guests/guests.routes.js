...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const guest = await Guest.findOne({ _id: req.params.id, qrcode: req.body.invitationCode, deleted: { $ne: true } }).populate({ path: 'event', populate: { path: 'host', select: 'accountType' } });
